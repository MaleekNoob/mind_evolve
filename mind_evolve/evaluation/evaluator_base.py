...
class SimpleScoreEvaluator(BaseEvaluator):
    ...
    def __init__(self, min_score: float = 0.0, max_score: float = 10.0, **kwargs: Any) -> None:
    ...
    def evaluate(self, solution_content: str, problem: Problem) -> EvaluationResult:
        ...
        if not has_content:
            ...
            score = self.min_score + (self.max_score - self.min_score) * length_factor * random.random()
            ...
            if not any(constraint.lower() in solution_content.lower() 
...
class ConstraintBasedEvaluator(BaseEvaluator):
    ...
    def _check_constraint(self, solution: str, constraint: str) -> bool:
        ...
        key_terms = [term.strip() for term in constraint_lower.split() 
                    if len(term.strip()) > 3 and term.strip() not in 
                    ['must', 'should', 'need', 'have', 'include', 'contain', 'require']]
    ...
    def parse_solution(self, solution_content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "word_count": len(solution_content.split()),
            "has_structure": any(line.strip().startswith(('-', '*', '1.', '2.')) 
                               for line in lines),
        }
